(function () {
    // FIX: font-weight: 999 is invalid CSS. The maximum is 900.
    // FIX: Bill styles now live exclusively here (removed from index.html's <style> block).
    const styles = `
...
.bill-total-value { font-size: clamp(2rem, 5.6vw, 3.6rem); line-height: 1; font-weight: 900; color: #000000; }
...
.bill-watermark {
    ...
    top: 20%;
    left: 20%;
    transform: translate(10%, 100px) rotate(-30deg);
    font-size: 4rem;
    opacity: 0.05;
    pointer-events: none;
    font-weight: 900;
...
@media print {
    ...
    #printable-bill {
        ...
        position: static !important; background: white !important; box-shadow: none !important;
...
.pdf-capture-mode #printable-bill {
    ...
    z-index: 99999 !important; height: auto !important; overflow: visible !important;
    display: block !important; background: white !important; box-shadow: none !important;
    border-radius: 0 !important; max-width: 100% !important; margin: 0 !important;
    padding: 16px !important; width: 210mm !important;
}
...
`;
    ...
    async function preloadAssets(appState) {
        if (appState.logoDataUri) return;
    ...
    // FIX: renderViewBill no longer mutates appState.currentView as a side-effect.
    // The caller (renderCurrentView in index.html) is responsible for guarding against a missing bill.
    // FIX: Inline onclick handlers replaced with data-action attributes.
    //      The main app's event delegation in index.html handles them.
            ...
            <div class="fade-in">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 no-print">
                    ...
                    <div class="flex flex-wrap gap-2 md:gap-3">
                        <button
                            data-action="nav-bills"
                            class="px-4 md:px-5 py-2.5 bg-gray-200 hover:bg-gray-300 rounded-xl font-semibold text-sm md:text-base transition-colors">
                            رجوع
                        </button>
                        <button
                            data-action="edit-bill"
                            class="px-4 md:px-5 py-2.5 rounded-xl font-semibold text-white flex items-center gap-2 text-sm md:text-base transition-all hover:opacity-90"
                        ...
                        <button
                            data-action="share-bill"
                            ${isLoading('sharing') ? 'disabled' : ''}
                            class="px-4 md:px-5 py-2.5 bg-blue-500 rounded-xl font-semibold text-white flex items-center gap-2 text-sm md:text-base">
                            ${isLoading('sharing') ? '<div class="loading-spinner"></div>' : '📤'}
                            ${isLoading('sharing') ? 'جاري التحضير...' : 'مشاركة PDF'}
                        </button>
                        <button
                            data-action="download-bill"
                            ${isLoading('downloading') ? 'disabled' : ''}
                            class="px-4 md:px-5 py-2.5 bg-green-600 rounded-xl font-semibold text-white flex items-center gap-2 text-sm md:text-base">
                            ${isLoading('downloading') ? '<div class="loading-spinner"></div>' : '⬇️'}
                            ${isLoading('downloading') ? 'جاري التحميل...' : 'تنزيل PDF'}
                        </button>
                    </div>
                </div>
                        ...
                        <div class="p-2 md:p-5 relative">
                            ...
                            <div class="bill-header mb-2">
                                <div class="bill-logo-frame h-24 md:h-32 w-24 md:w-32 shrink-0 bg-white rounded-xl shadow-sm">
                                    <img src="${logoSrc}" alt="شعار الشركة" class="bill-logo-image" />
                                </div>

                                <div class="text-right">
                                    <h2 class="text-xl md:text-3xl font-extrabold leading-tight"
                                        style="color: ${config.text_color};">
                                        ${escapeHtml(appState.companySettings.name)}
                                    </h2>
                                    <div class="flex flex-wrap gap-x-6 gap-y-1 mt-3 text-xs md:text-sm"
                                         style="color: #64748b;">
                                ...
                                <div class="text-left bg-gray-50 rounded-xl p-4 md:p-5 border"
                                     style="border-color: #e2e8f0;">
                                    <div class="mb-3">
                                        <p class="text-[10px] md:text-xs font-semibold mb-1"
                                           style="color: #94a3b8;">رقم الفاتورة</p>
                                        <p class="text-lg md:text-2xl font-extrabold"
                                           style="color: ${config.primary_color};">${bill.billNumber}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] md:text-xs font-semibold mb-1"
                                           style="color: #94a3b8;">التاريخ: ${formattedDate}</p>
                                    </div>
                                </div>
                            </div>

                            ${clientName ? `
                            <div class="mb-2 flex items-center gap-3 px-5 py-4 rounded-xl"
                                 style="background: linear-gradient(135deg, ${config.primary_color}08 0%, ${config.secondary_color}08 100%);
                                ...
                                <div>
                                    ...
                                    <p class="text-lg md:text-xl font-bold"
                                       style="color: ${config.primary_color};">أ/ ${escapeHtml(clientName)}</p>
                                </div>
                            </div>` : ''}
                                        ...
                                        ${bill.items.map(item => `
                                            <tr>
                                                <td class="text-center font-bold"
                                                    style="color: ${config.accent_color};">${item.total.toFixed(2)}</td>
                                                <td class="text-center font-semibold">${item.quantity.toFixed(2)}</td>
                                                <td class="text-center">${item.unitPrice.toFixed(2)}</td>
                                                <td class="text-center">
                                                    <span class="inline-block px-2 py-0.5 rounded-md text-xs font-semibold text-white"
                            ...
                            <div class="bill-footer-line mt-2 pt-2 text-center">
                                <p class="text-sm font-semibold" style="color: #64748b;">
                                    ${escapeHtml(appState.companySettings.footer)}
                                </p>
                                <p class="text-xs mt-2" style="color: #cbd5e0;">
                                    تم إنشاء هذه الفاتورة إلكترونيا - ${formattedDate}
                                </p>
                            </div>
                        </div>
                    </div>
